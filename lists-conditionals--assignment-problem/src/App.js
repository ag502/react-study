...
class App extends Component {
  ...
  deleteSelectedLetter = (event, idx) => {
    const word = [...this.state.inputText];
    word.splice(idx, 1);
    this.setState({ inputText: word.join("") });
  };
  ...
  render() {
    return (
      <div className="App">
        ...
        {this.state.inputText.split("").map((cur, idx) => {
          return (
            <CharComponent
              key={idx}
              letter={cur}
              click={event => this.deleteSelectedLetter(event, idx)}
            />
          );
        })}
      </div>
